...
export default function useDataDevoluciones(
  usuario?: string | null,
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      let q: any = supabase
        ...
        .select("total,fecha_devolucion,usuario,usuario_id");

      if (usuario) q = q.eq("usuario", usuario);
      if (usuario_id !== null && typeof usuario_id !== "undefined")
        q = q.eq("usuario_id", String(usuario_id));
