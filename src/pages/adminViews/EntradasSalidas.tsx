...
export default function EntradasSalidas() {
  ...
  async function loadRegistros() {
    ...
    try {
      ...
      if (sku || marca || categoria) {
        ...
        const filtered = Array.isArray(prodData)
          ? prodData.filter((p: any) => {
      ...
      let q2: any = supabase
        ...
        .select(
          "id, producto_id, cantidad, tipo_de_movimiento, referencia, usuario, fecha_salida"
        );
      ...
      const rows: Registro[] = Array.isArray(regData)
        ? regData.map((r: any) => ({
            id: r.id,
      ...
      const ids = Array.from(new Set(rows.map((r) => r.producto_id))).filter(
        Boolean
      );
      ...
      if (ids.length > 0) {
        const { data: prodData2 } = await supabase
          .from("inventario")
          .select("id, nombre, sku, marca, categoria, tipo")
          .in("id", ids)
      ...
      const filteredRows = rows.filter((r) =>
        map.hasOwnProperty(r.producto_id)
      );
  ...
  const entradas = registros.filter(
    (r) => String(r.tipo_de_movimiento).toUpperCase() === "ENTRADA"
  );
  const salidas = registros.filter(
    (r) => String(r.tipo_de_movimiento).toUpperCase() === "SALIDA"
  );
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      >
        ...
        <button className="btn-opaque" onClick={() => loadRegistros()}>
          Aplicar
        </button>
        <button
      ...
      <div style={{ display: "flex", gap: 12 }}>
        <div
          style={{ flex: 1, background: "#fff", padding: 12, borderRadius: 8 }}
        >
          <div
            style={{
              display: "flex",
                ...
                {entradas.map((r) => (
                  <tr key={String(r.id)}>
                    ...
                    <td style={{ minWidth: 120 }}>
                      {productosMap[r.producto_id]?.sku || r.producto_id}
                    </td>
