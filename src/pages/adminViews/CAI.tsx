...
export default function CAIView() {
  ...
  // compute assigned cajeros and cajas from current rows
  ...
  async function handleModalSave(id: number, payload: { cai?: string | null; rango_de?: string | null; rango_hasta?: string | null; fecha_vencimiento?: string | null; caja?: number | null; cajero?: string | null }) {
    ...
    setError(null)
    try {
      const { error } = await supabase.from('cai').update(payload).eq('id', id)
      if (error) throw error
      await loadCais()
    } catch (err: any) {
      setError(err?.message || String(err))
  ...
  return (
    <div style={{ padding: 18 }}>
      <h2 style={{ marginTop: 0 }}>Gestio de -CAI- por cajas</h2>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        <button className="btn-opaque" onClick={() => loadCais()}>Recargar</button>
      ...
      {/* prepare available options for create */}
      <CaiEditModal
        open={modalOpen}
        onClose={closeModal}
        row={modalRow}
      ...
      />
      <CaiCreateModal open={createOpen} onClose={() => setCreateOpen(false)} cajeros={cajeros} availableCajeros={cajeros.filter(c => !assignedCajeros.has(c.username))} availableCajas={[1,2,3,4,5].filter(n => !assignedCajas.has(n))} onCreate={async (payload) => {
        setLoading(true)
        setError(null)
        try {
          const { error } = await supabase.from('cai').insert(payload)
          if (error) throw error
          await loadCais()
          setCreateOpen(false)
        } catch (err: any) {
