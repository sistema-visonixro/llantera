...
export default function FacturasView() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sum = ventasPagadas.reduce((s: number, v: any) => s + Number(v.total || 0), 0);
      setTotalSum(sum);
      ...
      if (ventaIds.length > 0) {
        ...
        if (!detallesError && Array.isArray(detalles)) {
          const totalArt = detalles.reduce((s: number, d: any) => s + Number(d.cantidad || 0), 0);
    ...
    <div style={{ padding: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ background: 'white', padding: 16, borderRadius: 8 }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700 }}>{articulosVendidos}</div>
        </div>
      </div>
      ...
      <div style={{ background: 'white', padding: 12, borderRadius: 8 }}>
        ...
        {ventas.length === 0 && (
          <div style={{ padding: 32, textAlign: 'center', color: '#94a3b8' }}>No hay facturas en este rango</div>
        )}
      </div>
    </div>
  );
