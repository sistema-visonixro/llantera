...
export default function Stock() {
  ...
  async function loadRows() {
    ...
    setError(null)
    try {
      let query: any = supabase.from('inventario').select('id, nombre, sku, marca, modelo, descripcion, imagen')
      ...
      try {
        ...
        await Promise.all(inv.map(async (r) => {
          const raw = (r as any).imagen
          if (!raw) { urlMap[String(r.id)] = null; return }
          const src = String(raw)
          if (src.startsWith('http')) { urlMap[String(r.id)] = src; return }
          ...
          try {
            ...
            if (candidate) { urlMap[String(r.id)] = candidate; return }
          } catch (e) {
  ...
  return (
    <div style={{ padding: 18 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        <input className="input" placeholder="Filtro SKU" value={sku} onChange={e => setSku(e.target.value)} />
        <input className="input" placeholder="Filtro Marca" value={marca} onChange={e => setMarca(e.target.value)} />
        <input className="input" placeholder="Filtro Modelo" value={modelo} onChange={e => setModelo(e.target.value)} />
        <button className="btn-opaque" onClick={() => loadRows()}>Aplicar</button>
        <button className="btn-opaque" onClick={() => { setSku(''); setMarca(''); setModelo(''); loadRows() }}>Limpiar</button>
        <div style={{ marginLeft: 'auto', color: '#64748b' }}>{loading ? 'Cargando...' : `${rows.length} productos`}</div>
      </div>
      ...
      <div style={{ background: '#fff', padding: 12, borderRadius: 8, overflowX: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div></div>
          <div>
            <button className="btn-opaque" onClick={() => { document.body.classList.add('print-table-only'); setTimeout(() => { window.print(); document.body.classList.remove('print-table-only') }, 50) }}>Imprimir tabla</button>
          </div>
        </div>
              ...
              <tr key={r.id}>
                <td style={{ width: 70 }}>
                  {imageUrls[String(r.id)] ? (
                    <img src={encodeURI(imageUrls[String(r.id)] as string)} alt={r.nombre} style={{ width: 48, height: 48, objectFit: 'cover' }} onError={(e) => { (e.currentTarget as HTMLImageElement).src = ''; }} />
                  ) : (r.imagen ? <img src={r.imagen} alt={r.nombre} style={{ width: 48, height: 48, objectFit: 'cover' }} onError={(e) => { (e.currentTarget as HTMLImageElement).src = ''; }} /> : '')}
                </td>
                <td>{r.sku || ''}</td>
                <td>{r.marca || ''}</td>
                <td>{r.modelo || ''}</td>
                <td style={{ maxWidth: 320 }}>{r.descripcion || ''}</td>
                <td style={{ textAlign: 'right' }}>{stockMap[String(r.id)] != null ? String(stockMap[String(r.id)]) : '0'}</td>
              </tr>
            ))}
          </tbody>
        </table>
