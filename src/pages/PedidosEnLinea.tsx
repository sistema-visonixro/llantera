import React from 'react'

export default function PedidosEnLinea({ onBack }: { onBack: () => void }) {
  return (
    <div style={{ padding: 20, maxWidth: 1000, margin: '24px auto' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
        <h2 style={{ margin: 0 }}>Pedidos en Línea</h2>
        <button onClick={onBack} className="btn-opaque" style={{ padding: '8px 12px' }}>Volver</button>
      </header>

      <section style={{ background: 'white', padding: 16, borderRadius: 8, boxShadow: '0 1px 6px rgba(2,6,23,0.06)' }}>
        <p>Aquí se mostrarán los pedidos recibidos por canales en línea. Placeholder de ejemplo.</p>
        <p style={{ color: '#6b7280' }}>Integrar con API de pedidos cuando esté disponible.</p>
      </section>
    </div>
  )
}
