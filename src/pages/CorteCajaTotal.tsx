...
export default function CorteCajaTotal({ onBack }: { onBack: () => void }) {
  ...
  const {
    totals: devolucionesTotals,
    loading: devolucionesLoading,
    reload: reloadDevoluciones,
    ...
    <div style={{ padding: 20, maxWidth: 1000, margin: "24px auto" }}>
          ...
          >
            ...
            >
              ...
              <tbody>
                <BreakdownRow
                  label="💵 Efectivo"
                  ing={ingresos.efectivo}
                  anl={anulaciones.efectivo}
                  dev={devoluciones.efectivo}
                />
                <BreakdownRow
                  label="💳 Tarjeta"
                  ing={ingresos.tarjeta}
                  anl={anulaciones.tarjeta}
                  dev={devoluciones.tarjeta}
                />
                <BreakdownRow
                  label="🏦 Transfer"
                  ing={ingresos.transferencia}
                  anl={anulaciones.transferencia}
                  dev={devoluciones.transferencia}
                />
                <BreakdownRow
                  label="💵 Dólares"
                  ing={ingresos.dolares}
                  anl={anulaciones.dolares}
                  dev={devoluciones.dolares}
                />
                <tr
                  style={{
                ...
                >
                  ...
                  >
                    {currency(
                      ingresos.total - anulaciones.total - devoluciones.total
                    )}
                  </td>
                </tr>
              </tbody>
      ...
      >
        <div style={{ textAlign: "center" }}>
          <h3 style={{ marginTop: 0 }}>¿Estás seguro de cerrar la caja?</h3>
      ...
      <Confirmado
        ...
        message="La caja se cerró correctamente. Preparando reporte de cierre..."
        onClose={async () => {
          setSuccessOpen(false);
          // try to open a detailed print report similar to Parcial
          try {
            const now = new Date();
            const companyResp = await (
              await import("../lib/getCompanyData")
            ).default();
            const company = companyResp || {};
            ...
            const reportHtml = `
              <html>
                <head>
                  <meta charset="utf-8" />
                  <title>Reporte Cierre Total</title>
                  <style>body{font-family:Arial;padding:20px;color:#222}table{width:100%;border-collapse:collapse}th,td{padding:8px;border:1px solid #ddd;text-align:right}th{text-align:left;background:#f7fafc}</style>
                </head>
                <body>
                  <div style="text-align:center">${logoHtml}<h2>${
              company?.nombre_empresa || "Empresa"
            }</h2></div>
                  <h3>Reporte de Cierre Total</h3>
                  <div><strong>Cajero:</strong> ${session?.usuario}</div>
                  <div><strong>Apertura:</strong> ${
                  ...
                  <table style="margin-top:12px">
                    ...
                    <tbody>
                      <tr><td style="text-align:left">Total Ingresos</td><td>${
                        ingresos.total
                      }</td></tr>
                      <tr><td style="text-align:left">Total Egresos (desglosado)</td><td>${otrosEgresos}</td></tr>
                      <tr><td style="text-align:left">Devoluciones (total)</td><td>${
                        devoluciones.total
                      }</td></tr>
                      <tr><td style="text-align:left">Anulaciones (total)</td><td>${
                        anulaciones.total
                      }</td></tr>
                      <tr><td style="text-align:left">Saldo Final (teórico)</td><td>${saldoTeorico}</td></tr>
                    </tbody>
                  </table>
                </body>
              </html>
            ...
            console.warn("Error al generar reporte", e);
          }

          // refresh hooks and UI
          try {
            if (typeof (window as any).reloadAll === "function")
              (window as any).reloadAll();
          } catch (e) {}
          try {
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          fontWeight: 500,
          color: isPlus ? "#16a34a" : isMinus ? "#ef4444" : "#1e293b",
        }}
      >
        {isMinus ? "-" : ""}
        {currency(value)}
