...
export default function CompraDetailModal({ open, compraId, onClose }: { open: boolean; compraId: number | string | null; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      setLoading(true)
      try {
        const { data, error } = await supabase.from('compras_detalle').select('id, compra_id, producto_id, cantidad, costo_unitario, subtotal').eq('compra_id', compraId)
        if (error) throw error
        const detalles: DetalleRow[] = Array.isArray(data) ? data as DetalleRow[] : []
        if (!mounted) return
        setRows(detalles)

        const productoIds = Array.from(new Set(detalles.map(d => d.producto_id))).filter(Boolean)
        if (productoIds.length > 0) {
          const { data: prods } = await supabase.from('inventario').select('id, nombre, exento').in('id', productoIds)
          const map: Record<string, Producto> = {}
          const exmap: Record<string, boolean> = {}
          if (Array.isArray(prods)) prods.forEach((p: any) => { map[String(p.id)] = { id: p.id, nombre: p.nombre }; exmap[String(p.id)] = (p.exento === true || p.exento === 1 || String(p.exento).toLowerCase() === 'true') })
          if (mounted) {
            setProductosMap(map)
            setExentoMap(exmap)
          }
        ...
        try {
          ...
          const impuestoNumber = impuestoRow ? Number(impuestoRow.impuesto_venta) : 0
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}>
      <div style={{ width: 760, maxWidth: '98%', background: '#fff', borderRadius: 8, padding: 16, maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <table className="admin-table">
              ...
              <tbody>
                {rows.map(r => {
                  ...
                  const subtotalLine = isExento ? totalLine : totalLine - impuestoLine
                  return (
                    <tr key={String(r.id)}>
              ...
              <tfoot>
                {rows.length > 0 && (() => {
                  const totals = rows.reduce((acc, r) => {
                    ...
                    const subtotalLine = isExento ? lineTotal : lineTotal - impuestoLine
